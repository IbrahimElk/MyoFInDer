...
if __name__ == '__main__':
    ...
    chunk_size = len(final_result) // 4
    chunks = [final_result[i:i + chunk_size]
    ...
    labeled_images = pool.map(process_chunk, chunks)

    # Close the pool of processes and wait for all tasks to complete
    pool.close()
    pool.join()
    # Merge the results from all chunks into the final labeled_image
    # labeled_image = np.vstack(labeled_images)

    t7 = time.perf_counter()
    print("Total time for predicting all images in different processes:", t7 - t6)
